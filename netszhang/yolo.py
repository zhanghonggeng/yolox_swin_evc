...
class YOLOXHead(nn.Module):
    def __init__(self, num_classes, width = 0.375, in_channels = [128, 256, 512, 1024], act = "silu", depthwise = False,):
        ...
        for i in range(len(in_channels)):
            ...
            self.stems.append(BaseConv(in_channels = int(in_channels[i] * width), out_channels = int(self.headsize * width), ksize = 1, stride = 1, act = act))
            self.cls_convs.append(nn.Sequential(*[
                Conv(in_channels = int(self.headsize * width), out_channels = int(self.headsize * width), ksize = 3, stride = 1, act = act),
                Conv(in_channels = int(self.headsize * width), out_channels = int(self.headsize * width), ksize = 3, stride = 1, act = act),
            ]))
            self.cls_preds.append(
                nn.Conv2d(in_channels = int(self.headsize * width), out_channels = num_classes, kernel_size = 1, stride = 1, padding = 0)
            )
...
class YOLOPAFPN(nn.Module):
    def __init__(self, depth = 0.33, width = 0.375, in_features = ("dark2", "dark3", "dark4", "dark5"), in_channels = [128, 256, 512, 1024], depthwise = False, act = "silu"):
        super().__init__()
        Conv                = DWConv if depthwise else BaseConv
