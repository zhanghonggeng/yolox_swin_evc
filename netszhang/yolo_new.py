...
from nets.darknet_baseline import BaseConv, CSPDarknet, CSPLayer, DWConv


class YOLOXHead(nn.Module):
    def __init__(self, num_classes, width=0.375, in_channels=[128, 256, 512, 1024], act="silu", depthwise=False, ):
        super().__init__()
...
class YOLOPAFPN(nn.Module):
    def __init__(self, depth=0.33, width=0.375, in_features=("dark2", "dark3", "dark4", "dark5"),
        ...
        self.reduce_conv3 = BaseConv(int(in_channels[2] * width), int(in_channels[1] * width), 1, 1, act=act)
        self.reduce_conv4 = BaseConv(int(in_channels[3] * width), int(in_channels[2] * width), 1, 1, act=act)
        # -------------------------------------------#
    ...
    def forward(self, input):
        ...
        # print('dddddddddd', P5_upsample.shape)

        # #-------------------------------------------#
        # #   120, 120, 512 -> 120, 120, 256
        # #-------------------------------------------#
        # P4          = self.reduce_conv1(P5_upsample)
        P4 = self.reduce_conv1(self.C4_p4(torch.cat([self.upsample(P5), feat3], 1)))
        # print('P4',P4.shape)
        ...
        P5_cat = self.upsample1(P5)
        P5_cat = self.C4_p2(self.C4_p3(P5_cat))
        P3_P5 = self.reduce_conv2(torch.cat([P5_cat, P3], 1))
        # print('P5P3',P3_P5.shape)
        # print('P3',P3.shape)
        ...
        P2_out = self.C4_p2(torch.cat([self.upsample(P3_P5), feat1], 1))
        P5_cat1 = self.upsample2(P5)
        P5_cat1 = self.C4_p2(self.C4_p3(P5_cat1))
        P2_P5 = self.reduce_conv2(torch.cat([P5_cat1, P2_out], 1))
        # print('P2_P5',P2_P5.shape)
        # print('P2OUT',P2_out.shape)
        # #-------------------------------------------#
        # #   480, 480, 128 -> 240, 240, 128
        # #-------------------------------------------#
        # P2_downsample   = self.bu_conv0(P2_out)
        # print('11111111',P2_downsample.shape)
        # #-------------------------------------------#
        ...
        p3_w1 = self.p3_w1_relu(self.p3_w1)
        weight = p3_w1 / (torch.sum(p3_w1, dim=0) + self.epsilon)
        P3_out = self.C4_n3(weight[0] * feat2 + weight[1] * torch.cat([self.bu_conv0(P2_P5), P3_P5], 1))
        P5_P3_out = self.upsample1(P5)
        P5_P3_out = self.C4_p3(P5_P3_out)
        P5_P3_out = self.reduce_conv3(torch.cat([P5_P3_out, P3_out], 1))
        print('P5_P3_out',P5_P3_out.shape)
        #
        #
        # print('P3OUT',P3_out.shape)
        # exit()
        ...
        p4_w1 = self.p4_w1_relu(self.p4_w1)
        weight = p4_w1 / (torch.sum(p4_w1, dim=0) + self.epsilon)
        P4_out = self.C4_n4(weight[0] * feat3 + weight[1] * torch.cat([self.bu_conv1(P5_P3_out), P4], 1))
        P5_P4_out = self.upsample(P5)
        P5_P4_out = self.reduce_conv4(torch.cat([P5_P4_out,P4_out],1))
        # print('P4OUT',P4_out.shape)
        # print('P5_P4_out',P5_P4_out.shape)
        # exit()
        # #-------------------------------------------#
        # #   120, 120, 512 -> 60, 60, 512
        # #-------------------------------------------#
        ...
        P5_out = self.C4_n5(torch.cat([self.bu_conv2(P5_P4_out), P5], 1))
        # print('P5OUT', P5_out.shape)
        # exit()
        return (P2_P5, P5_P3_out, P5_P4_out, P5_out)
...
if __name__ == '__main__':
    model = YoloBody(10, 'l')
    # print(model)
    test_tensor = torch.randn([1, 3, 1600, 1600])
