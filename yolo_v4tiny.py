...
from nets_v4tiny.yolo import YoloBody
from utils_v4tiny.utils import (cvtColor, get_anchors, get_classes, preprocess_input,
                                resize_image, show_config)
from utils_v4tiny.utils_bbox import DecodeBox, DecodeBoxNP
...
class YOLO(object):
    _defaults = {
        ...
        # --------------------------------------------------------------------------#
        "model_path": 'logs_v4tiny/best_epoch_weights.pth',
    ...
    #   检测图片
    # ---------------------------------------------------#
    def detect_image(self, image, img_name, dir_save_path, crop=False, count=False):
        ...
        for i, c in list(enumerate(top_label)):
            ...
            y2 = "{:.1f}".format(bottom)
            x2 = "{:.1f}".format(right)
            Task_1txt = open(os.path.join(dir_save_path, "Task2_%s.txt" % predicted_class), 'a', encoding='utf-8')
            Task_1txt.write("%s %s %s %s %s %s\n" % (img_name, score, x1, y1, x2, y2))
            Task_1txt.close()
...
class YOLO_ONNX(object):
    ...
    def detect_image(self, image):
        ...
        feature_map_shape = [[int(j / (2 ** (i + 4))) for j in self.input_shape] for i in
